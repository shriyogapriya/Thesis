...
\begin{document}
	...
	The energy of an ensemble of structures is calculated using a partition function \citep{mccaskill1990equilibrium} . Similarly, we get $E_{i,k}^{unpaired}$ by calculating the partition function, ${Z_s}_{i,k}^{unpaired}$ where $s_{i,k}^{unpaired} $ is an ensemble of all structures which can be formed by a sequence S, with a single stranded region $[i,k]$. Therefore,
	
		\begin{center}
		
		$E_{i,k}^{unpaired} = - RTln({Z_s}_{i,k}^{unpaired})$
	...
	\section{Iterative scheme for multisite}
	 ...
	 \item Step 2: Then we will derive the 'blocking' constraint from step 1 by rerunning IntaRNA and predict conditional minimum free energy and site. Here we block $B_1$ (Constrainted IntaRNA) and we get $B_2'$ as minimum free energy.
	 \item Step 3: Thirdly, the energy of an respective M-RRI can be computed from the two energies. ie., the energy of the conditional call can be added using the conditional probability.\\
	 \begin{center}
	 	 $E(B_1 \land B_2) = E(B_1) + E(B_2 | B_1)$ .
	 \end{center}
	 
	 \item Step 4: Since prediction (2) is conditional, the existence of the interaction from (2) can have effects on (1). Thus, one starts to iterate the procedure from (2) but fixes the conditional site. Then , we print the respective the $B_1$ and $B_2$.
	 \item Step 5: Finally, check for convergence: is the site from two-steps before retained? If yes: convergence and stop iteration by printing the new $B_1 , B_2$. If no: repeat constraint prediction until convergence by swapping ( ie.,$B_2 = B_1; B_1 = B_2'$ ).
	...
	E(B_1 \land B_2) = E(B_1) + E(B_2 | B_1)	
		\end{equation}
		\begin{center}
	$E(B_1 \land B_2) = E_{hyb}(B_1 \land B_2) + ED(B_1 \land B_2)$
		\end{center}
	\begin{equation}
	\label{2}
	E(B_1 \land B_2) = E_{hyb}(B_1)+E_{hyb}(B_2)-RTlog(\mathcal{P}^u(B_1 \land B_2))
	\end{equation}
	From Eqn ~\ref{1}, 
	\begin{center}
	...
	\begin{equation}
	\label{3}
	E(B_1) = E_{hyb}(B_1) - RTlog(\mathcal{P}^u(B_1))
	\end{equation}
	\begin{center}
	...
	\begin{equation}
	\label{4}
	E(B_2 | B_1) = E_{hyb}(B_2) - RTlog(\mathcal{P}^u(B_2 | B_1))
	\end{equation}
	
	
	From Eqn ~\ref{2} = Eqn ~\ref{3} + Eqn ~\ref{4}
	 
	\begin{equation}
	\label{5}
	E_{hyb}(B_1)+E_{hyb}(B_2)-RTlog(\mathcal{P}^u(B_1 \land B_2)) \Leftrightarrow E_{hyb}(B_1) + E_{hyb}(B_2) - RTlog(\mathcal{P}^u(B_1)) - RTlog(\mathcal{P}^u(B_2 | B_1))
	\end{equation}
	 
	As we know $log (A) + log (B) = log( A . B)$ , we apply this condition in right end side of the Eqn ~\ref{5}\\
	...
	\end{center}
 	Therefore right end side of the Eqn ~\ref{5} becomes,
 	\begin{equation}
 	\label{6}
 	E_{hyb}(B_1) + E_{hyb}(B_2) - RTlog(\mathcal{P}^u(B_1 \land B_2))
 	\end{equation}
	
	Now,  Eqn ~\ref{2} = Eqn ~\ref{6},
	\begin{equation}
	\label{7}
